...
public class PaymentController {
    ...
    @PostMapping
    @Transactional
    public ResponseEntity makePayment(@RequestBody PaymentDTO paymentDTO, HttpServletRequest request) {
        ModelAndView modelAndView = new ModelAndView();
        ...
        if (checkUserRole()) {
            try {
                ...
                orderDTO.setTime(new Time(Calendar.getInstance().getTime().getTime()));
                orderDTO.setDescription(paymentDTO.getService());
                orderDTO.setValue(paymentDTO.getPrice());
